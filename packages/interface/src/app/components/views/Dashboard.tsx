...
function Dashboard() {
    ...
    const depositView = deposits.map(item => {
        return (
            <div key={item.asset} className="grid grid-cols-3 pl-4 pr-16 py-2 border border-gray border-opacity-50 border-t-0">
    ...
    const debtView = debts.map( debt => {
            return (
                <div key={debt.asset} className="grid grid-cols-3 py-2 pl-4 border border-gray border-opacity-50 border-t-0">
                    <div className="text-left flex space-x-2">
                        <div><img src={debt.assetIcon} alt="" /></div>
                        <div>{debt.asset}</div>
                    </div>
                    <div className="text-left">{debt.value}</div>
                    <div className="text-left pl-4">{debt.interest}</div>
                </div>
            )
    });
    ...
    const basic = (
        <div className="basic-panel space-y-4">
            <div className="flex justify-between px-10 pt-10 items-center">
                ...
                <div className="space-y-2">
                    <div className="opacity-50 text-lg">Current Liquidation Threshold</div>
    ...
    return (
        <div className="dashboard max-w-7xl mx-auto px-6">
            <div className="md:grid grid-cols-3 mt-10 text-left gap-8 space-y-4 md:space-y-0">
                <div className="col-span-1 bg-secondary overflow-hidden">
                
                    <div className="absolute space-y-1 pt-4 pl-4 object-cover">
                    ...
                    { isProtected && <img src={shield} alt="protect" className="relative float-right object-none object-bottom mt-28"/> }
            ...
            <div className="flex mt-10 space-x-8">
                <div className="space-y-4">
                    <div className="opacity-50 text-left">YOUR DEPOSITS</div>
                    <div>
                ...
                <div className="space-y-4">
                    <div className="opacity-50 text-left">YOUR DEBTS</div>
                    <div>
                        <div className="flex justify-between space-x-24 bg-secondary pl-4 pr-16 py-2 border border-gray border-opacity-50 rounded-t-md font-bold">
                            <div>Assets</div>
                            <div>Value</div>
